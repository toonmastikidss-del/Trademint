...
import { ChevronLeft, QrCode, Info, Upload, Image as ImageIcon, X } from 'lucide-react';
import axios from 'axios';
import { API_CONFIG } from '../config/apiConfig';

const UmoneyQRPayment = () => {
      ...
      try {
        ...
        if (generalQr) {
          ...
          if (generalQr.upiId) {
            ...
            setDynamicUpiId('umoney@merchant123'); // Fallback
          }
        }
      } catch (error) {
        console.error('Error fetching QR code:', error);
        setDynamicUpiId('umoney@merchant123'); // Fallback on error
      } finally {
        setQrLoading(false);
      }
    };
  ...
  const handleCopyUPI = () => {
    const upiIdToCopy = dynamicUpiId || 'umoney@merchant123';
  ...
  return (
    <div className="bg-[#101821] min-h-screen text-white pb-24 font-sans">
      {/* Header */}
      <div className="sticky top-0 z-50 bg-purple-600 px-4 py-4 flex items-center justify-between">
        <button onClick={() => navigate('/deposite')} className="p-1">
          <ChevronLeft size={24} className="text-white" />
        </button>
        <h1 className="text-lg font-bold tracking-tight">Umoney-QR Payment</h1>
      ...
      <div className="px-4 mt-6 space-y-6">
        ...
        <div className="bg-[#212431] border border-gray-700 rounded-3xl p-6 shadow-xl">
          ...
          <div className="flex items-center justify-between">
            <span className="text-3xl font-black text-purple-500">₹{amount}</span>
        ...
        <div className="bg-[#212431] border border-gray-700 rounded-3xl p-6 shadow-xl text-center">
          ...
          <div className="text-left space-y-3 text-sm text-gray-400">
            <p>1. For iOS phones, long press the QR code and enter Umoney wallet</p>
            <p>2. For Android phones, take a screenshot and manually open Umoney wallet to upload the QR code</p>
        ...
        <div className="bg-[#212431] border border-gray-700 rounded-3xl p-6 shadow-xl">
          ...
          {/* Step 1: Copy UPI */}
          <div className="mb-6">
            <p className="text-sm text-gray-400 mb-2">1. Copy the below given Umoney ID</p>
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="bg-purple-600 text-white px-4 py-3 rounded-xl font-bold text-sm hover:bg-purple-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {copied ? 'COPIED' : 'COPY'}
              </button>
            </div>
            <p className="text-xs text-orange-400 mt-2">Dont save the Umoney ID, get new ID every time.</p>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-orange-400 mt-2">
              Open your Umoney wallet and complete the transfer. Record your reference No. (Ref No.) after payment.
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#212431] border border-gray-700 rounded-3xl p-6 shadow-xl">
          ...
          <ul className="space-y-2 text-sm text-gray-400">
            <li className="flex items-start gap-2">
              ...
              <span>Please select the payment method you need and make sure your phone has Umoney wallet installed.</span>
            </li>
          </ul>
        </div>
      </div>
...
export default UmoneyQRPayment;
